...
static void on_play_again_clicked     (void *state) { *(enum State *)state = STATE_PLAY; }
	...
	struct SDL_Window *wnd, const struct EllipsoidPic *winnerpic)
{
	...
	char msg[100];
	basename_without_extension(winnerpic->path, msg, sizeof(msg)/2);
	strcat(msg, " wins!");
	SDL_Surface *winnertext = create_text_surface(msg, white_color, 60);

	enum State state = STATE_GAMEOVER;
	...
	blit_with_center(winnertext, wndsurf, &(SDL_Point){ wndsurf->w/2, wndsurf->h/4 });
